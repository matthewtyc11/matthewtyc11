...
function updateTradeItems(id) {
    ...
    api.updateShopItemForPlayer(id, 'trade', 'viewYourItems', { userInput: { type: 'dropdown', dropdownOptions: (t[me + 'Items'] ?? []).map((x, y) => `${y + 1}: ${x.amt} ${x?.attributes?.customDisplayName ?? x.name}`) } })
    api.updateShopItemForPlayer(id, 'trade', 'viewTheirItems', { userInput: { type: 'dropdown', dropdownOptions: (t[them + 'Items'] ?? []).map((x, y) => `${y + 1}: ${x.amt} ${x?.attributes?.customDisplayName ?? x.name}`) } })
    api.updateShopItemForPlayer(id, 'trade', 'removeItem', { userInput: { type: 'dropdown', dropdownOptions: (t[me + 'Items'] ?? []).map((x, y) => `${y + 1}: ${x.amt} ${x?.attributes?.customDisplayName ?? x.name}`) } })
}
onPlayerBoughtShopItem = (id, cKey, iKey, item, input) => {
    if (cKey === 'trade') {
        if (iKey === 'sendRequest') {
            ...
            api.sendMessage(them, `${api.getEntityName(id)} has added an item to the trade!`, { color: 'lime' })
            ...
            api.sendMessage(them, `${api.getEntityName(id)} has removed an item from the trade!`, { color: 'red' })
        }
    }
}
tick = dt => {
    ...
    for (let i = trades.length - 1; i >= 0; i--) {
        ...
        if (t.timer === 0) {
            ...
            api.sendOverShopInfo(t.p1, `Trade executed in ${(confirmTimer - t.timer) / 1000}s!`)
            api.sendOverShopInfo(t.p2, `Trade executed in ${(confirmTimer - t.timer) / 1000}s!`)
        }
        t.timer += dt
